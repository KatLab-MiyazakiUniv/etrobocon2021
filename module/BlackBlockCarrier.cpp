...
void BlackBlockCarrier::carryBlackBlock()
{
  ...
  //青の線を通過
  lineTracer.run(250, 20, RUN_PWM, RUN_GAIN);
  //青の線まで
  lineTracer.run(650, TARGET_BRIGHTNESS, RUN_PWM, CURVE_GAIN);
  //青の線を通過
  lineTracer.run(350, 20, RUN_PWM, RUN_GAIN);
  //黄色の円まで
  lineTracer.runToColor(TARGET_BRIGHTNESS, RUN_PWM, RUN_GAIN);
  //黄色の円を通過
  straightRunner.runStraightToDistance(120, RUN_PWM + 10);
  //緑の円まで
  lineTracer.runToColor(TARGET_BRIGHTNESS, RUN_PWM, RUN_GAIN);
  //緑の円の中心まで移動
  straightRunner.runStraightToDistance(RUN_PWM, 55);
  //右に９０度回頭
  ...
  while(Mileage::calculateWheelMileage(measurer.getRightCount()) + startDiff
    ...
    controller.setLeftMotorPwm(43);
    controller.sleep();
  }
  controller.stopMotor();

  //ビンゴの中心まで直進
  straightRunner.runStraightToDistance(400, RUN_PWM - 20);
  //黒のラインまで下がる
